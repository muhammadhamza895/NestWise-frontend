...
import { searchProperties, getLocationTrends, analyzeProperties } from "../services/api";
import {
  Building,
  MapPin,
  TrendingUp,
...
import SearchTrend from "../components/ai/SearchTrend";
...
const AIPropertyHub = () => {
  ...
  const [loadingTrendStage, setLoadingTrendStage] = useState("");
  const [loadingTime, setLoadingTime] = useState(0);
  const [loadingTrendTime, setLoadingTrendTime] = useState(0);
  ...
  useEffect(() => {
    ...
    if (isTrendLoading) {
      interval = setInterval(() => {
        setLoadingTrendTime((prev) => prev + 1);
      }, 1000);
    } else {
      setLoadingTrendTime(0);
      setLoadingTrendStage("");

      // Scroll to results when loading completes and results are available
      if (searchTrendPerformed && contentTrendRef.current) {
        setTimeout(() => {
          contentTrendRef.current.scrollIntoView({
            behavior: "smooth",
            block: "start",
          });
        }, 100);
  ...
  const handleTrendSearch = async (searchParams) => {
    setIsTrendLoading(true);
    setSearchError("");
    setSearchTrendPerformed(true);
    setLoadingTrendTime(0);

    try {
      // Fetch location trends for the same city
      setLoadingTrendStage("locations");
      const locationResponse = await getLocationTrends(searchParams.city, searchParams.purpose);
  ...
  const renderTrendLoadingIndicator = () => {
    const getLoadingMessage = () => {
      if (loadingTrendTime < 5) {
        return "Gathering market trend data from various sources...";
      } else if (loadingTrendTime < 15) {
        return "Initializing AI model for real estate trend analysis...";
      } else if (loadingTrendTime < 30) {
        return "Analyzing historical property prices and market patterns...";
      } else {
        return "Finalizing insights and generating market forecasts for you...";
      }
    };

    const getProgressWidth = () => {
      const progress = Math.min(95, (loadingTrendTime / 45) * 100);
      return `${progress}%`;
    };
    ...
    return (
      <div className="flex flex-col items-center justify-center py-8 sm:py-12">
        ...
        <div className="bg-blue-50 border border-blue-100 p-4 sm:p-5 rounded-lg max-w-xs sm:max-w-md shadow-md mx-4 sm:mx-0">
          ...
          <p className="text-blue-700 text-xs sm:text-sm">
            {loadingTrendTime < 20
              ? "Analyzing regional sales data, property cycles, and neighborhood activity to uncover market trends."
              : "AI is evaluating long-term price movements, seasonality patterns, and growth potential to generate actionable insights."}
          </p>
          ...
          {loadingTrendTime > 15 && (
            <div className="mt-3 pt-3 border-t border-blue-100">
              <div className="flex items-center gap-2">
                <div className="w-1.5 h-1.5 sm:w-2 sm:h-2 bg-blue-600 rounded-full animate-pulse"></div>
                <p className="text-xs text-blue-600 font-medium">
                  Deep market intelligence takes time to synthesize
                </p>
              </div>
            </div>
          )}
  ...
  return (
    <div className="min-h-screen bg-gray-50 pt-16 sm:pt-20 pb-8 sm:pb-12">
      ...
      <div className="container mx-auto px-4">
          ...
          {!isLoading && searchPerformed && (
            <div className="space-y-8 sm:space-y-12">
              ...
              <div className="bg-white rounded-lg sm:rounded-xl shadow-md p-4 sm:p-6 md:p-8">
                ...
                {!isAnalysisLoading && propertyAnalysis && (
        ...
        {(isLoading || isAnalysisLoading) && renderLoadingIndicator()}
          ...
          <div className="bg-white p-5 sm:p-8 rounded-lg sm:rounded-xl shadow-md max-w-4xl mx-auto text-center">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4 sm:gap-6 mt-6 sm:mt-8">
              ...
              <FeatureCard
                ...
                description="Get recommendations on property investment potential"
              />
            </div>
          </div>
        </div>
        ...
        <div className="mt-16 mb-8 sm:mb-12 bg-gradient-to-r from-blue-600 to-indigo-700 rounded-xl sm:rounded-2xl p-5 sm:p-8 shadow-lg sm:shadow-xl">
          <div className="max-w-3xl mx-auto text-center">
            <h1 className="text-3xl sm:text-4xl font-bold text-white mb-3 sm:mb-4">
              Real Estate Trending Location Tool
            </h1>
            <p className="text-blue-100 text-lg sm:text-xl mb-6 sm:mb-8">
              Uncover the latest property trends.
            </p>

            <div className="bg-white/95 backdrop-blur rounded-lg sm:rounded-xl p-4 sm:p-6 shadow-md sm:shadow-lg">
              <SearchTrend onSearch={handleTrendSearch} isTrendLoading={isTrendLoading} />
            </div>
          </div>
        </div>

        {isTrendLoading && renderTrendLoadingIndicator()}

        <div ref={contentTrendRef}>
          {!isTrendLoading && searchTrendPerformed && (
            <div className="space-y-8 sm:space-y-12">
              {/* Location Trends Section */}
              <div className="bg-white rounded-lg sm:rounded-xl shadow-md p-4 sm:p-6 md:p-8">
...
const FeatureCard = ({ icon, title, description }) => (
  <div className="bg-gray-50 p-4 sm:p-5 rounded-lg border border-gray-100">
    <div className="text-blue-600 mb-2 sm:mb-3 w-fit mx-auto">{icon}</div>
