...
import { checkIsAdmin } from "../../services/checkUser.js";
...
const PropertyDetails = () => {
  ...
  const [isCloseDealLoading, setIsCloseDealLoading] = useState(false);
  ...
  const scheduleViewing = () => {
    ...
    if (isAdmin) return toast.error("Oops! Admins don’t need to schedule viewings.")
    setShowSchedule(true)
  }

  const closeDeal = async () => {
    const isAdmin = checkIsAdmin()
    if (isAdmin) return toast.error("Oops! Admins can't close the deal")

    try {
      if (!isLoggedIn) {
        toast.error('Please login to close a deal');
        return;
      }

      const token = localStorage.getItem('token');

      setIsCloseDealLoading(true);
      const response = await axios.post(
        `${Backendurl}/api/deal/close-deal`,
        {
      ...
      if (response.data.success) {
        toast.success("CLose deal request sent successfully")
      }
    } catch (error) {
      console.error('Scheduling error:', error);
      const errorMessage = error.response?.data?.message || 'Error sending close deal request';
      toast.error(errorMessage);
    } finally {
      setIsCloseDealLoading(false);
    }
  }
