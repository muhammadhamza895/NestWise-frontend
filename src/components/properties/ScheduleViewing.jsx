...
import { Calendar, Clock, Loader, X, Info, CheckCircle, Users, MapPin, Building } from 'lucide-react';
import { motion, AnimatePresence } from 'framer-motion';
import axios from 'axios';
import { toast } from 'react-toastify';
import { Backendurl } from '../../App';

const ScheduleViewing = ({ propertyId, adminId, propertyTitle, propertyLocation, propertyImage, onClose }) => {
  const [formData, setFormData] = useState({
    date: '',
    time: '',
    notes: '',
    appointmentPurpose : 'Property Viewing'
  });
  ...
  const fetchTimeSlots = async (date) => {
    try {
      setLoading(true);
      const response = await axios.get(`${Backendurl}/api/appointments/time-slots?adminId=${adminId}&date=${date}`);

      if (response.data.success) {
        const bookedSlots = response.data.appointments;
        setBookedTimeSlots(bookedSlots)
      } else {
        setError(response.data.message || "Failed to Fetch available time slots");
      }
    } catch (err) {
                        ...
                        >
                          ...
                          {timeSlots.map((slot) => (
                            <option
                              ...
                              value={slot}
                              disabled={
                                (formData.date === dateRestrictions.min && isPastTime(slot)) ||
                                (bookedTimeSlots?.includes(slot))
                              }
                              className={bookedTimeSlots?.includes(slot) ? "text-red-500" : ""}
                            >
                              {slot}
                            </option>
                          ))}
                  ...
                  >
                    ...
                    <div>
                      ...
                      <select
                        ...
                        onChange={(e) => setFormData(prev => ({ ...prev, appointmentPurpose: e.target.value }))}
